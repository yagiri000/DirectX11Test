...
void Game::Render()
{
	...
	postProcess->SetEffect(BasicPostProcess::GaussianBlur_5x5);
	postProcess->SetSourceTexture(sceneSRV.Get());
	postProcess->Process(m_context.Get());
}
...
void Game::Clear()
{
	...
	m_context->ClearRenderTargetView(sceneRT.Get(), Colors::CornflowerBlue);
	m_context->ClearDepthStencilView(m_depthStencilView.Get(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
	m_context->OMSetRenderTargets(1, sceneRT.GetAddressOf(), m_depthStencilView.Get());
...
void Game::CreateDevice()
{
	...
	DX::ThrowIfFailed(
		m_device->CreateTexture2D(&sceneDesc, nullptr, sceneTex.GetAddressOf())
	);

	DX::ThrowIfFailed(
		m_device->CreateShaderResourceView(sceneTex.Get(), nullptr,
			sceneSRV.ReleaseAndGetAddressOf())
	);

	DX::ThrowIfFailed(
		m_device->CreateRenderTargetView(sceneTex.Get(), nullptr,
	...
	DX::ThrowIfFailed(
		CreateWICTextureFromFile(m_device.Get(), L"hito.png", resource.GetAddressOf(), m_texture.ReleaseAndGetAddressOf())
	);
	...
	postProcess = std::make_unique<BasicPostProcess>(m_device.Get());
}
